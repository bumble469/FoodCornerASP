        ...
        protected void loginbtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (connection.State == ConnectionState.Closed)
                    connection.Open();
                SqlCommand sqlCommand = new SqlCommand("SELECT * FROM users WHERE email = @EmailId AND password = @PassWord", connection);
                sqlCommand.Parameters.AddWithValue("@EmailId", (object)this.userEmailid.Text.Trim());
                ...
                if (sqlDataReader.HasRows)
                {
                    while (sqlDataReader.Read())
                    {
                        ...
                        this.Session["u_id"] = (object)sqlDataReader.GetValue(0).ToString();
                        ...
                        this.Session["fname"] = (object)sqlDataReader.GetValue(1).ToString();
                        this.Response.Redirect("UserVerifyPage.aspx");
                    }
                }
                else
                {
